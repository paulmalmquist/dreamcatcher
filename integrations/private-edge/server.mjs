...
export function createHandler({controlOrigin,galleryOrigin,edgeKey,getRoutes,fetcher=fetch,now=Date.now}){
 ...
 return async(req,res)=>{
  ...
  try{
   ...
   for(const [key,session] of sessions)if(session.expires_at*1000<=now())sessions.delete(key);
   if(url.pathname==='/_dc/launch'){
    ...
    if(sessions.size>=5000){res.writeHead(503);res.end('Try again shortly');return}
    ...
    sessions.set(id,{...result,appId:route.appId,origin:route.origin});
   ...
   const session=cookies.length===1?sessions.get(cookies[0].slice('__Host-dc_app='.length)):undefined;
   if(!session||session.appId!==route.appId||session.origin!==route.origin){res.writeHead(401);res.end('Session ended. Launch again from Dreamcatcher.');return}
...
if(process.argv[1]&&import.meta.url===pathToFileURL(process.argv[1]).href){
 ...
 const handler=createHandler({controlOrigin:env.DC_CONTROL_ORIGIN,galleryOrigin:env.DC_GALLERY_ORIGIN,
  edgeKey:env.DC_WORKER_EDGE_KEY??'',getRoutes:async()=>JSON.parse(await readFile(env.DC_EDGE_ROUTES,'utf8'))});
