...
export class DreamcatcherError extends Error {
 constructor(public readonly status:number,public readonly details:unknown,public readonly retryAfterSeconds?:number){super(typeof details==='string'?details:JSON.stringify(details));this.name='DreamcatcherError'}
}
export function createDreamcatcher(options:ClientOptions){
 ...
 async function request<T>(path:string,body?:unknown,method=body?'POST':'GET',signal?:AbortSignal):Promise<T>{
  ...
  const response=await fetcher(new URL('/api'+path,base),{method,headers:{Authorization:'Bearer '+token,...(body?{'Content-Type':'application/json'}:{})},body:body?JSON.stringify(body):undefined,redirect:'error',signal:signal?AbortSignal.any([timeout,signal]):timeout});
  ...
  if(!response.ok){const retry=response.headers.get('retry-after');throw new DreamcatcherError(response.status,value.detail??value,retry&&/^\d+$/.test(retry)?Math.min(3600,Number(retry)):undefined)}return value as T;
 }
 ...
 return {
  ...
  queries:{run:<T=Record<string,unknown>>(reference:Reference,parameters:Record<string,unknown>,callOptions?:{signal?:AbortSignal})=>request<QueryResult<T>>('/execute/query',{app_id:options.appId,reference,parameters},'POST',callOptions?.signal)},
