...
- The separate Docker/Chromium story is defined in `tests/e2e/story.py`; use its GitHub Actions run and `summary.json` for its actual outcome. It stops at the first failure. See `docs/CONTAINER-STORY.md` for the real-versus-mocked evidence boundary. Adding the test alone is not a passed browser/container claim.
